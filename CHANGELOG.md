## [1.0.5](https://github.com/collidor/schema-command/compare/v1.0.4...v1.0.5) (2026-09-24)


### Bug Fixes

* **deps:** update @collidor/command to ^7.1.0 ([6f9d26d](https://github.com/collidor/schema-command/commit/6f9d26d57db2d6af04a64dc2b4606be13fbaaea4))

## [1.0.4](https://github.com/collidor/schema-command/compare/v1.0.3...v1.0.4) (2026-08-16)


### Bug Fixes

* trigger patch release ([7f1d4f9](https://github.com/collidor/schema-command/commit/7f1d4f97d02bdb562fb829730619bd39207d793f))

## [1.0.3](https://github.com/collidor/schema-command/compare/v1.0.2...v1.0.3) (2026-08-16)


### Bug Fixes

* **ci:** add dummy NPM_TOKEN to satisfy semantic-release preflight check for OIDC ([d2bc1e0](https://github.com/collidor/schema-command/commit/d2bc1e0ded96362e163704b85ac16f683e81a171))
* **ci:** decouple npm and jsr publish from semantic-release ([6e17993](https://github.com/collidor/schema-command/commit/6e17993d8c57b70eadfcf2db60cd9a80dda3ed47))
* **ci:** re-enable native semantic-release npm publishing for OIDC ([a901000](https://github.com/collidor/schema-command/commit/a901000d767fb7134c34e398a568e6521bc7a3c6))

## [1.0.2](https://github.com/collidor/schema-command/compare/v1.0.1...v1.0.2) (2026-08-15)


### Bug Fixes

* **ci:** add --allow-dirty to jsr publish to ignore uncommitted lockfiles ([16d155a](https://github.com/collidor/schema-command/commit/16d155a698fc202c4f9023c792912e40cac4c12a))

## [1.0.1](https://github.com/collidor/schema-command/compare/v1.0.0...v1.0.1) (2026-08-15)


### Bug Fixes

* **ci:** rename workflow file to publish.yml to match npm trusted publisher configuration ([aec20a3](https://github.com/collidor/schema-command/commit/aec20a30512636bc3adf021e78a8ae776ab8a39c))

# 1.0.0 (2026-08-15)


### Bug Fixes

* **build:** add @swc/core devDependency required for tsup es5 build target ([0683318](https://github.com/collidor/schema-command/commit/06833180b8f0f4118618801649396a539d7c5746))
* **ci:** configure npmPublish false and provenance publishCmd for tokenless npm OIDC, fix repo URL format ([61d35e6](https://github.com/collidor/schema-command/commit/61d35e6f1c6275036723cad75b80b4523eeeea79))
* **ci:** update release pipeline to use OIDC trusted publishing without tokens ([5c23a3f](https://github.com/collidor/schema-command/commit/5c23a3fc529de2cec435b95e3f3ef46c64b855fd))


### Features

* add automated semantic-release pipeline for npm and jsr ([8b1b5a7](https://github.com/collidor/schema-command/commit/8b1b5a7eacb948b1add7034528454d35570e37e7))

# @collidor/command

## 0.1.4

### Patch Changes

- fix imports
- Updated dependencies
  - @collidor/command@7.0.1

## 0.1.3

### Patch Changes

- Y
- Updated dependencies
  - @collidor/command@7.0.0

## 0.1.2

### Patch Changes

- Updated dependencies
  - @collidor/command@6.0.3

## 0.1.1

### Patch Changes

- Updated dependencies
  - @collidor/command@6.0.2

## 0.1.0

### Minor Changes

- SchemaCommand now has zod schema for output

## 0.0.7

### Patch Changes

- Create result and fix dependencies sync
- Updated dependencies
  - @collidor/command@6.0.1

## 0.0.6

### Patch Changes

- Add cleanup callback to event.on return
- Updated dependencies
  - @collidor/command@6.0.0

## 0.0.5

### Patch Changes

- update tsup and export injector register type
- Updated dependencies
  - @collidor/command@5.3.15

## 0.0.4

### Patch Changes

- Reupload
- Updated dependencies
  - @collidor/command@5.3.14

## 0.0.3

### Patch Changes

- Update readme and fix npmrc

## 0.0.2

### Patch Changes

- add schema toolkit
- Updated dependencies
  - @collidor/command@5.3.13

## 0.0.2

### Patch Changes

- 677ef8f: first package
- 80e351b: add command to toolkit
- Updated dependencies [a8b57a3]
- Updated dependencies [80e351b]
  - @collidor/command@5.3.12

## 5.3.11

### Patch Changes

- publish
- Updated dependencies
  - @collidor/event@4.2.10

## 5.3.10

### Patch Changes

- publish test
- Updated dependencies
  - @collidor/event@4.2.9

## 5.3.9

### Patch Changes

- test publish
- Updated dependencies
  - @collidor/event@4.2.8

## 5.3.8

### Patch Changes

- 09d97fb: publish test
- publish test
- Updated dependencies [09d97fb]
- Updated dependencies
  - @collidor/event@4.2.7

## 5.3.7

### Patch Changes

- 17c8fc2: publish test
- publish
- Updated dependencies [17c8fc2]
- Updated dependencies
- Updated dependencies [17c8fc2]
  - @collidor/event@4.2.6

## 5.3.6

### Patch Changes

- publish
- Updated dependencies
  - @collidor/event@4.2.5

## 5.3.5

### Patch Changes

- 750cba9: publish
- Updated dependencies [750cba9]
  - @collidor/event@4.2.4

## 5.3.4

### Patch Changes

- publish
- Updated dependencies
  - @collidor/event@4.2.3

## 5.3.3

### Patch Changes

- publish
- 9f45da4: publish
- Updated dependencies
- Updated dependencies [9f45da4]
  - @collidor/event@4.2.2
